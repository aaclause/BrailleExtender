...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def getCurrentSelectionRange(self, pretty=True, back=False):
		if pretty:
			labels = [
				_('Character'),
				_('Word'),
	...
	def script_volumePlus(s, g):
		...
		s = '%3d%%%s' % (utils.getVolume(), utils.translatePercent(
			utils.getVolume(), braille.handler.displaySize - 4))
		braille.handler.message(
			s) if configBE.conf['general']['reportVolumeBraille'] else None
		speech.speakMessage(str(
			utils.getVolume())) if configBE.conf['general']['reportVolumeSpeech'] else None
		return
			...
			if 'kb:' in g and 'capsLock' not in g and 'insert' not in g:
				...
				doc += u'<li>{0}{2}: {1}{2};</li>'.format(
					gt.replace(
						...
						lst[g]).replace(
						'br(' + configBE.curBD + '):', ''), configBE.sep)
			else:
				if isinstance(lst[g], list):
	...
	def getDoc(s):
		...
		if configBE.gesturesFileExists:
			...
			doc += s.translateLst({k: configBE.iniProfile["miscs"][k] for k in configBE.iniProfile["miscs"]
								   if 'rotor' in k.lower()}) + s.translateLst(configBE.iniProfile["rotor"])
			doc += ('<h2>' + _('Gadget commands') +
					' (%s)</h2>') % str(len(configBE.iniProfile["miscs"]) - 2)
			doc += s.translateLst(OrderedDict([(k, configBE.iniProfile["miscs"][k])
		...
		doc += ('<h2>' + _('Shortcuts on system keyboard specific to the add-on') +
				' (%s)</h2>') % str(len(s.__gestures) - 4)
		doc += '<ul>'
		for g in [k for k in s.__gestures if k.lower().startswith('kb:')]:
			if g.lower() not in [
				'kb:volumeup',
				'kb:volumedown',
					'kb:volumemute'] and s.__gestures[g] not in ['logFieldsAtCursor']:
				doc += (u'<li>{0}{1}: {2}{3};</li>').format(
					utils.getKeysTranslation(g),
					configBE.sep,
					re.sub(
	...
	def script_quickLaunch(self, gesture):
		...
		try:
			return Popen(configBE.quickLaunchS[configBE.quickLaunch.index(
	...
	def sendCombKeys(self, sendKS, send=True):
		if send:
			...
			KeyboardInputGesture.fromName(
				sendKS).send() if not sendKS == "" else None
		return
	...
	def script_end_combKeysChar(self, gesture):
		...
		if not hasattr(gesture, 'id'):
			...
			self.sendComb(
				self.getActualModifiers(False) + utils.bkToChar(
					gesture.dots,
					brailleTables.listTables()[
						configBE.conf['general']['iTableSht']][0]),
				gesture) if not configBE.noUnicodeTable and configBE.conf['general']['iTableSht'] > -1 and configBE.conf['general']['iTableSht'] < len(
				brailleTables.listTables()) else self.sendComb(
					self.getActualModifiers(False) + utils.bkToChar(
						gesture.dots),
				gesture)
		self.clearModifiers()
	...
	def initCombKeys(self):
		self.bindGestures(
			self._tGestures) if self.lenModifiers() == 1 else None
