...
AdvancedInputModeDictEntry = namedtuple("AdvancedInputModeDictEntry", ("abreviation", "replaceBy", "table"))
...
def initialize():
	...
	for entry in json_:
		entries.append(AdvancedInputModeDictEntry(entry["abreviation"], entry["replaceBy"], entry["table"]))
...
def saveDict(entries=None):
	if not entries: entries = getDictionary()
	entries = [{"abreviation": entry.abreviation, "replaceBy": entry.replaceBy, "table": entry.table} for entry in entries]
...
def getReplacements(abreviations, strict=False):
	if isinstance(abreviations, str): abreviations = [abreviations]
	...
	for abreviation in abreviations:
		...
		if not strict:
			out += [entry for entry in getDictionary() if entry.abreviation.startswith(abreviation) and entry.table in [currentInputTable, '*']]
		else:
			out += [entry for entry in getDictionary() if entry.abreviation == abreviation and entry.table in [currentInputTable, '*']]
	return out

def translateTable(tableFilename):
	if tableFilename == '*': return _("all tables")
	else:
		for table in brailleTables.listTables():
			if table.fileName == tableFilename: return table.displayName
...
class AdvancedInputModeDlg(gui.settingsDialogs.SettingsDialog):
	...
	def makeSettings(self, settingsSizer):
		...
		self.dictList = sHelper.addLabeledControl(entriesLabelText, wx.ListCtrl, style=wx.LC_REPORT | wx.LC_SINGLE_SEL,
												  size=(550, 350))
	...
	def onSetEntries(self):
		...
		for entry in self.tmpDict:
			self.dictList.Append((entry.abreviation, entry.replaceBy, translateTable(entry.table)))
	...
	def onOpenFileClick(self, event):
		...
		if not os.path.exists(dictPath): return ui.message(_("File doesn't exist yet"))
	...
	def postInit(self): self.dictList.SetFocus()
...
class DictionaryEntryDlg(wx.Dialog):
	...
	def __init__(self, parent=None, title=_("Edit Dictionary Entry")):
		...
		self.replaceByTextCtrl = sHelper.addLabeledControl(replaceByLabelText, wx.TextCtrl)
