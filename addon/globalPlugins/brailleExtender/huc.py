...
def translateHUC6(dots, debug=False):
	...
	for l1, l2 in zip(linedCells1, linedCells2):
		...
		i += 1
	if debug: print_(":translateHUC6:", dots, "->", repr(out))
	out = '-'.join(out)
	return out
...
def translateHUC8(dots, debug=False):
	...
	out = ''.join(sorted(out))
	if debug: print_(":translateHUC8:", dots, "->", out)
	return out
...
def splitInTwoCells(dotPattern):
	...
	for dot in dotPattern:
		if dot in ['3', '6', '7', '8']: c2 += dot
		elif dot in ['1', '2', '4', '5']: c1 += dot
	if c2: c2 = translateHUC8(c2)
	if not c1: c1 = '0'
	if not c2: c2 = '0'
	return [c1, c2]
...
def isValidHUCInput(s):
	...
	if size == 2: return HUC_INPUT_COMPLETE
	elif size < 2: return HUC_INPUT_INCOMPLETE
	return HUC_INPUT_INVALID

def backTranslateHUC8(s, debug=False):
	if len(s) not in [3, 4]: raise ValueError("Invalid size")
	prefix = s[0:2] if len(s) == 4 else s[0]
	s = s[2:] if len(s) == 4 else s[1:]
	if prefix not in HUC8_patterns.keys(): raise ValueError("Invalid prefix")
	out = []
