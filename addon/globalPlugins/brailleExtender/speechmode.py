...
# the speech option can not be positioned such that the user goes directly from it to "automaticly" when using the NVDA + ctrl + t command. If it is, the braille display does not show the focus propperly until the user either switches to another tether option or moves the focus.
...
def scrollBack(self):
	...
	oldScrollBack(self)
	if braille.handler.buffer == braille.handler.mainBuffer and braille.handler.getTether() == "speech" and braille.handler.buffer.windowRawText == windowRawText:
		global index
		if index>0:
			index = index-1
		showSpeech(index)
